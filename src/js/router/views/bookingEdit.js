...
async function prefillEditForm() {
  try {
    ...
    try {
      const { data: venueWithBookings } = await readVenue(booking.venue.id, {includeBookings: true});
      venueBookings = venueWithBookings.bookings || [];
    } catch(err) {
    ...
    if (booking.venue) {
      venueDetails.innerHTML = `
        <div class="bg-white p-6 rounded-2xl shadow-lg">
          <h2 class="text-2xl font-bold text-[var(--brand-purple)] mb-2">${
            booking.venue.name
          }</h2>
          <p class="text-gray-700">${booking.venue.description || ""}</p>
          <p class="text-gray-500 text-sm">${
            booking.venue.location?.city || ""
          }, ${booking.venue.location?.country || ""}</p>
        </div>
      `;
    }
...
form.addEventListener("submit", async (e) => {
  ...
  const conflictBooking = venueBookings.find((b) => {
  ...
  if (conflictBooking) {
    displayBanner(
      `Selected dates conflict with an existing booking: ${formatDate(
        conflictBooking.dateFrom
      )} – ${formatDate(conflictBooking.dateTo)}`,
      "error"
    );
    return;
  }
